...
int AX12A::readPositionNew(unsigned char ID){
  /* Returns a positive position value,
   * or an error code multiplied by -1 in the range [-127, -1] (see error codes here in http://www.trossenrobotics.com/images/productdownloads/AX-12(English).pdf at page 11),
   * or one of the additional error codes:
   * -128 if did not receive even two bytes in response in the set timeout,
   * -129 if did not find packet header 0xFFFF in the set timeout,
   * -130 if did not get the full packet in the set timeout,
   * -131 if the checksum of the packet is incorrect.
   */
  // Send instruction packed
  const unsigned int length = 8;
  unsigned char packet[length];
  ...
  // Get status packed
  const unsigned long max_waiting_time_ms = 10;
  unsigned long end_time = millis() + max_waiting_time_ms;  // may overflow if arduino is up for more than 50 days

  unsigned char second_byte;
  unsigned char status_id;
  unsigned char status_length;
  unsigned char pos_low_byte;
  unsigned char pos_high_byte;
  unsigned char true_checksum;
  unsigned char their_checksum;

  while(availableData() < 2) {
    if (millis() > end_time) return (-128);  // No response
    delayus(1000);
  }
  Incoming_Byte = readData();
  second_byte = readData();

  while ((Incoming_Byte != 255) | (second_byte != 255)) {
    while(availableData() < 1) {
      if (millis() > end_time) return (-129);  // No header
      delayus(1000);
    }
    Incoming_Byte = second_byte;
    second_byte = readData();
  }

  while(availableData() < 6) {
    if (millis() > end_time) return (-130);  // No full packet
    delayus(1000);
  }
  
  status_id = readData();
  status_length = readData();
  Error_Byte = readData();
  pos_low_byte = readData();
  pos_high_byte = readData();
  their_checksum = readData();

  true_checksum = (~(status_id + status_length + Error_Byte + pos_low_byte + pos_high_byte)) & 0xFF;    
  if (their_checksum != true_checksum) return (-131);  // Incorrect checksum.
  if (Error_Byte != 0) return (Error_Byte * (-1));  // Error.
  
  Position_Long_Byte = pos_high_byte << 8;
  Position_Long_Byte = Position_Long_Byte + pos_low_byte;
...
//}
//
//void println_hex(unsigned char X) {
//   if (X < 16) {Serial.print("0");}
//   Serial.println(X, HEX);
//}
//
//void print_packet(unsigned char h1, unsigned char h2, unsigned char id, unsigned char len, unsigned char er, unsigned char p1, unsigned char p2, unsigned char chk){
//  print_hex(h1);
//  print_hex(h2);
//  print_hex(id);
//  print_hex(len);
...
//int AX12A::readPositionNew(unsigned char ID){
//  /* Returns a positive position value,
//   * or an error code in the range [-127, -1] (see error codes here in http://www.trossenrobotics.com/images/productdownloads/AX-12(English).pdf at page 11),
//   * or one of the additional error codes:
//   * -128 if did not receive enough bytes in the set timeout,
//   * -129 if did not find packet header (0xFFFF) in the set timeout,
//   * -130 if the checksum of the packet is incorrect.
//   */
//  const unsigned int length = 8;
//  unsigned char packet[length];
//  
...
//
//  Time_Counter = 0;
//  while((availableData() < 8) & (Time_Counter < TIME_OUT)){
//    Time_Counter++;
//    delayus(1000);
//  }
//
//  if (availableData() < 8) return (-128);  // Not enough data.
//  
//  while (availableData() >= 8) {
//    Incoming_Byte = readData();
//    second_byte = peekData();
//    Serial.print("Header?: ");
//    print_hex(Incoming_Byte);
//    println_hex(second_byte);
//    if ((Incoming_Byte != 255) | (second_byte != 255)) continue;
//  
//    second_byte = readData();
//    status_id = readData();
//    status_length = readData();
...
//
//    true_checksum = (~(status_id + status_length + Error_Byte + pos_low_byte + pos_high_byte)) & 0xFF;    
//    if (their_checksum != true_checksum) return (-130);  // Incorrect checksum.
//    if (Error_Byte != 0) return (Error_Byte * (-1));  // Error.
//    
//    Position_Long_Byte = pos_high_byte << 8;
//    Position_Long_Byte = Position_Long_Byte + pos_low_byte;
//    print_packet(Incoming_Byte, second_byte, status_id, status_length, Error_Byte, pos_low_byte, pos_high_byte, their_checksum);
//    return Position_Long_Byte;
//  }
//  return (-129);  // No packet header.
//}
