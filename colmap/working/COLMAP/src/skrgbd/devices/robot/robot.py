...
from skrgbd.utils.math import make_extrinsics
...
class Robot(ABC):
    ...
    def lookat(self, pos, lookat, up):
        ...
        """
        tpose = self._lookat(pos, lookat, up)
        return self.rob.movel(tpose, acc=self.acceleration, vel=self.velocity)

    def _lookat(self, pos, lookat, up):
        r"""Calculate parameters to move TCP to `pos` and look to `lookat`.
    ...
    def get_point_id(self, *coords):
        ...
        return f'{self.trajectory_name}@' + ','.join(coord_strs)
